            ...
            <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
                ...
                ? props.filteredSpecialities.map((item, index) => (
                        <Grid size={6} key={index} className='mt-5 shadow-md rounded-md hover:shadow-xl transition ease-in duration-300 hover:-translate-y-1 hover:scale-105 pb-5'>
                            <div className=''>
                                <img src={item.picture} alt="" className='block m-auto rounded-t-md md:h-80 w-full h-40'/>
                                ...
                                <Link href={`/courses/${item.id}`} className='p-2 bg-indigo-500 text-white rounded-md hover:shadow-lg mt-2 block m-auto text-center md:w-1/2 w-3/4'>Kursni ko'rish</Link>
                            </div>
                        </Grid>
                    ))
